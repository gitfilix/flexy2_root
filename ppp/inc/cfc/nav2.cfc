		...
		<CFLOOP query="objNav">
				...
				<CFIF checkForKids.recordcount gt 0 AND (objNav.userGroupID EQ "" OR objnav.usergroupid EQ arguments.usergroup) AND allowed EQ 1><!--- this child has kids too! add it to the menustring, then make the recursive call... --->
					...
					<cfsavecontent variable="variables.menustring3">
						...
						<li>
						<a href="#trim(rewriteLinkById(id=objnav.id))#" <cfif objnav.id EQ listLast(session.navtree)>class="active"</cfif> <cfif arguments.level EQ 4>style="padding-left:10px;"</cfif>>
							#objNav.NavDescription#
						</a>
					</cfsavecontent>
				...
				<CFELSEif allowed EQ 1><!--- this child is childless...just add it to the menustring... --->
					...
					<cfsavecontent variable="variables.menustring3">
						...
						<li>
						<a href="#trim(rewriteLinkById(id=objnav.id))#" <cfif objnav.id EQ listLast(session.navtree)>class="active"</cfif> <cfif arguments.level EQ 3>style="margin-left:18px;"</cfif>>
							#objNav.NavDescription#
						</a>
					</cfsavecontent>
