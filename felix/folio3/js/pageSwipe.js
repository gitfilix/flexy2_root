...
(function( $ ) {
  ...
  $.fn.pageSwipe = function( options ) {
	...
	initSlide = function(cp,csp){
		...
		// parallax-Init
		/*b = $('section','.page-'+cp).data('parallax')
		if(b && csp==0){
			$('#bg3').css({
				
				background: 'url('+$('section','.page-'+cp).data('parallaxpanorama')+')',
				width: $('section','.page-'+cp).data('parallaxwidth'),
				height: '100%',
				position: 'absolute',
				top: 0,
				left: 0,
	...
	scrollToPage = function(pagenr,subpagenr,sec,type) {
		...
		if(type=='slide'){
			that[0].style.WebkitTransition = 'all '+sec+'s linear'
			...
			// parallax-Init
			/*b = $('section','.page-'+currentPage).data('parallax')
			if(b && currentSubPage==0){
				$('#bg3').css({
					
					background: 'url('+$('section','.page-'+currentPage).data('parallaxpanorama')+')',
					width: $('section','.page-'+currentPage).data('parallaxwidth'),
					height: '100%',
					position: 'absolute',
					top: 0,
					left: 0,
			...
			parallaX = ($('section','.page-'+$('bg3').data('startslide')).data('parallaxwidth') - deviceWidth) / ($('section','.page-'+currentPage).data('parallaxslides')-1)
			parallaxStartPos = $('section','.page-'+pagenr).position().left
			
			if(pagenr>currentPage && currentSubPage == 0 && subpagenr == 0){
			
				dir = "fw"	
				$('#bg3').css({
				   '-webkit-transition': 'all '+sec+'s linear',
				   '-webkit-transform': "translateX(" + (parallaxStartPos-parallaX) + "px)",   
				   '-moz-transition': 'all '+sec+'s linear',
				   '-moz-transform': "translateX(" + (parallaxStartPos-parallaX) + "px)"   
				})
			}
			else if(currentSubPage == 0 && subpagenr == 0){
	...
	isWithinParallax = function(pagenr){
		if($('#bg3').css('background')!=''){
			return true	
		}
		else{
			return false	
     ...
     if (!touchDown) {
      ...
      if (scrollDirection == "h") {
		...
		// if on subpage, clone the page and set it to the top, for smooth (but unlogic) sliding
		  ...
		  /*parallaX = ($('#bg3').data('parallaxwidth') - deviceWidth) / ($('#bg3').data('parallaxslides')-1)
		  parallaxStartPos = $('#bg3').offset().left
		  parallaxOffset = deviceWidth / parallaX
		  
		  
		  if(horizontalDirection == "left"){
				$('#bg3').css({
				   '-webkit-transform': 'translate3d(' + (pagesStartPosition + (horizontalTouchDelta*0.05)) + 'px,0,0)',
				   '-moz-transform': "translateX(" + (pagesStartPosition + (horizontalTouchDelta*0.05)) + "px)"
			   })
		  }
		  else{
			   $('#bg3').css({
				   '-webkit-transform': 'translate3d(' + ((pagesStartPosition+deviceWidth) + (horizontalTouchDelta/2)) + 'px,0,0)',
				   '-moz-transform': "translateX(" + ((pagesStartPosition+deviceWidth) + (horizontalTouchDelta/2)) + "px)"
			   })
			}*/
		 // $('#bg4').css('WebkitTransform', 'translate3d(' + (pagesStartPosition + (horizontalTouchDelta/4)) + 'px,0,0)')
	...
	$('.pages').bind('touchend.pageswipe mouseup.pageswipe', function(event) {
		...
		if (scrollDirection == "h") {
			...
			if (horizontalTouchDelta < 80 && horizontalTouchDelta > -80 && z == 0 && z2 == 0 && z3 == 0 && z4 == 0 && goIntoCSSTransform == 1) {
				...
				//alert(currentPage);
				that[0].style.WebkitTransition = 'all 0.3s linear'
				...
				//$('#bg3').css('WebkitTransform', 'translate3d(' + (nextPositionH/2) + 'px,0,0)')
