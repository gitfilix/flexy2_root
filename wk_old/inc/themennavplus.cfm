...
<ul class="stripe">
	...
	<cfoutput query="getThemenNav">
		...
		<cfif id EQ session.startID AND ownDomain NEQ "">
			<a href="http://www.#ownDomain#" <cfif id EQ listFirst(session.navtree)>class="themennav_active" style="color:##910000 !important; "</cfif>>
				#pagetitel#
			</a>
		<!--- jeder andere link --->
		<cfelse>
			<a href="#trim(application.com.nav.rewriteLinkById(id=id,lang=lang))#" <cfif id EQ listFirst(session.navtree)>class="themennav_active" style="color:##910000 !important; "</cfif>>
				#pagetitel#
			</a>
		</cfif>
